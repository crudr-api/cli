...
var Exec = function( program ){
	...
	// init lib
	this.api = new CRUDr( this.config );
...
Exec.prototype = {
	...
	user: function( id, options ){
		...
		if( options.apps ) params.type = "apps";
		if( options.stores ) params.type = "stores";

		this.get( params );
	},

	app: function( id, options ){
		var params = {};
		if( id ) params.id = id ;

		// read the product collection for the current user: /apps
		params.name = ( options.list ) ? "apps" : "app";
	...
	store: function( id, options ){
		...
		// read the product collection for the current user: /stores
		params.name = ( options.list ) ? "stores" : "store";
